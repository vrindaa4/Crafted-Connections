...
app.get('/editcategory', async function (req, res) {
	...
	res.render('editcategory_view', { category: result });
})
...
app.post('/addItemSubmit', async function (req, res) {
	...
	const result = await itemObj.insertOne({
		...
		pSold: parseInt(req.body.pSold),
		pQuantity: parseInt(req.body.pQuantity),
		pCategory: req.body.pCategory,
		pOffer: req.body.pOffer,
		pRating: parseFloat(req.body.pRating),
...
app.post('/addUserSubmit', async function (req, res) {
	...
	const result = await usersCollection.insertOne({
		...
		phoneNumber: req.body.phoneNumber,
		verified: req.body.verified === 'on', // 'on' indicates checkbox is checked
		secretKey: req.body.secretKey,
		address: req.body.address,
		invoice: req.body.invoice // Assuming you want to store invoice as well
	});
...
app.post('/editUserSubmit', async function (req, res) {
	try {
		...
		// Safely handle undefined 'history'
		const history = req.body.history ? req.body.history.split(",") : [];
...
app.listen(8000, () => console.log("CRUD Server running at http://localhost:8000/"));
